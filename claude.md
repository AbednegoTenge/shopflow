...
`deployment_circuit_breaker { enable = true, rollback = true }` added to `aws_ecs_service.app` so a bad CI deploy auto-rolls-back instead of looping forever burning failed-task cycles — this incident is exactly the scenario it guards against. Applied and confirmed live (`terraform plan` clean).
**ECS-side deploy worked after the Buildx fix, but the GitHub Actions job still showed red** — the deployment rolled out cleanly (0 failed tasks, revision 6, `COMPLETED`, `/health` verified live), yet the workflow run itself reported `conclusion: failure`. Cause: the **Smoke test** step calls `aws elbv2 describe-load-balancers` to find the ALB's DNS name, but `github_deploy`'s IAM policy never granted `elasticloadbalancing:DescribeLoadBalancers` — every prior deploy step (ECR push, register/deploy) has its own scoped permission, but nobody had scoped one for the post-deploy smoke test. The call silently failed (stderr, not captured by `$(...)`), `$ALB_DNS` came back empty, every retry curled `http://health` and got nothing. Fixed by adding an `elasticloadbalancing:DescribeLoadBalancers` statement to `cicd/iam.tf` — `Resource = "*"` since, like `ecs:DescribeTaskDefinition`, this describe/list action doesn't support resource-level scoping.
**Lesson:** ECS/service health and "the GitHub Actions job passed" are two different claims — checking one doesn't confirm the other. Always check the actual workflow run conclusion, not just whether the deployed thing looks healthy.
**Confirmed fully green after the IAM fix** (run `32656762244`, commit `69996f8`): every step succeeded including Smoke test, `conclusion: success`. Three real bugs found and fixed to get here — OIDC `sub` claim's immutable-ID suffix, amd64/ARM64 image mismatch, missing `elasticloadbalancing:DescribeLoadBalancers` — none of them hypothetical, all found by actually running the pipeline rather than just reading the YAML. Phase 6 is done, verified, for real this time.
...
**Full destroy/rebuild history:** destroyed and rebuilt three times so far, always to stop idle resources from running up cost between sessions — 2026-08-21/22 (mid-Phase-3), 2026-08-23 once Phase 5 code existed (6-stage staged apply: `networking` → `data` → ECR alone → image build/push → rest of `compute` → `async` → `edge`), and again 2026-08-23 end-of-day after Phase 6 was done and verified (`terraform destroy`, 78 resources, clean — state confirmed empty via `terraform state list`). A `terraform destroy` can leave orphaned state for resources with dependent-object protection (e.g. ECR repos aren't force-deleted by default — fixed for good this time by adding `force_delete = true` to `compute/ecr.tf` before the third destroy, so this shouldn't recur) — if a resource shows in `terraform state list` but `aws <service> describe-*` 404s, it's orphaned state, not a real resource; `terraform state rm` it before re-applying rather than trying to import or recreate. **All infra is currently torn down** — nothing is running, `terraform state list` is empty. To resume: same staged sequence as before (`networking` → `data` → ECR → image push → `compute` → `async` → `edge` → `cicd`), full details of each stage's known quirks are in the Phase-by-phase notes above. The GitHub OIDC provider is real AWS account-level state, not project state — expect the next `terraform apply` on `module.cicd` to either create it fresh (if this rebuild is the first thing to touch it) or need the same import dance as before if something else in the account created one first. If any resource ID, ARN, or endpoint referenced in older notes/screenshots doesn't match live AWS, trust live AWS — current IDs are the ones above until the next rebuild changes them (`terraform output` after re-applying will have the new ones).
...
Phases 3 through 6 are code-complete and were verified live before being torn down. **Infra is currently fully destroyed** (see destroy/rebuild history above) — first step next session is the staged rebuild (`networking` → `data` → ECR → image push → `compute` → `async` → `edge` → `cicd`), not new feature work. Once rebuilt, pick up Phase 7 (observability): CloudWatch dashboard for ALB latency/5xx, ECS CPU/memory, RDS connections, SQS depth; alarms → SNS → email; X-Ray on the ALB and ECS task; explicit log retention on the CloudWatch log group (currently unset — "never expire").
