...
**Worth doing before Phase 7:** add `deployment_circuit_breaker { enable = true, rollback = true }` to `aws_ecs_service.app` so a bad CI deploy auto-rolls-back instead of looping forever burning failed-task cycles — this incident is exactly the scenario that guards against.
Re-tested after the Buildx fix — not yet confirmed passing end to end as of this note; see git history / latest workflow run for current status.
